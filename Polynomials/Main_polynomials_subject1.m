...
q = dummy_motion.data(:,[5:9,18,15:17]).*(pi/180);
...
if runPolynomialfit
    ...
    for m = 1:length(muscleNames)
        ...
        MuscleData.dM(:,m,7)    = MA.trunk.ext.data(:,strcmp(lMT.colheaders,muscleNames{m}));   % knee
        MuscleData.dM(:,m,8)    = MA.trunk.ben.data(:,strcmp(lMT.colheaders,muscleNames{m}));   % ankle
        MuscleData.dM(:,m,9)    = MA.trunk.rot.data(:,strcmp(lMT.colheaders,muscleNames{m}));   % sub 
    end
    MuscleData.q = q;

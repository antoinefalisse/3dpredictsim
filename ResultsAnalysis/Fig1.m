...
ylim_P = [-50,200;-100,100;-150,150;-400,400];
NumTicks_P = 2;
for i = 1:length(idx_Qs)
    subplot(NSubPlots(1),NSubPlots(2),pos_P(i))
    % Simulation results
    for k = 1:length(ww)
        x = 1:(100-1)/(size(Qdots_opt(ww(k)).m,1)-1):100;
        plot(x,Qdots_opt(ww(k)).m(:,idx_Qs(i)).*pi/180.*...
            Ts_opt(ww(k)).m(:,idx_Qs(i))*body_mass,...
            'color',color_all(k,:),'linewidth',line_linewidth);
        hold on;    
    end
    % Experimental data
    idx_jref = strcmp(Pref.(subject).colheaders,joints_ref{i});

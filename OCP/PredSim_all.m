...
idx_ww = 1;%:198; % Index row in matrix settings (1:198)
...
if decomposeCost
    ...
    threshold = 20;
    GRF_recombined = [GRF_opt_unsc(:,2);GRF_opt_unsc(:,5)];
    last_noContact = find(GRF_recombined<threshold,1,'last');
    if last_noContact == 2*N
        first_contact = find(GRF_recombined>threshold,1,'first');
    else
        first_contact = last_noContact + 1;
    end
    if first_contact > N
        IC1i_test = first_contact - N;
        HS1_test = 'l';
    else
        IC1i_test = first_contact;
        HS1_test = 'r';
    end
        
        
    test_contact(ww,1:2) = [IC1i,IC1i_test];
    test_contact_cell{ww,1} = HS1;
    test_contact_cell{ww,2} = HS1_test;

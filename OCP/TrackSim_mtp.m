...
for p = 1:length(trials)
    nametrial(p).id    = ['gait_',trials{p},'_mtp']; % Experimental walking trial to track
...
for p = 1:length(trials)
    [bounds(p).p,scaling(p).p] = ...
        getBounds_tracking_mtp(Qs(p).p,NMuscle,nq,jointi,dev_cm,GRF(p).p);
end
...
for p = 1:length(trials)
    guess(p).p = ...
        getGuess_DI_tracking_mtp(Qs(p).p,nq,N,NMuscle,jointi,scaling(p).p);
end
...
if solveProblem
    ...
    paramsCM_nsc    = f_nsc18(paramsCM,scaling(1).p.params.v,scaling(1).p.params.r);        
    for p = 1:length(trials)    
...
%         end
    ...
    %% Unscale results
    % Parameters
    paramsCM_opt_nsc = f_nsc18(paramsCM_opt,scaling(1).p.params.v,scaling(1).p.params.r);
    for p = 1:length(trials)   

...
tl = load([pathpolynomial,'/muscle_spanning_joint_INFO_',subject,'_',...
        mtp_jointType,'.mat']);
...
load([pathpolynomial,'/muscle_spanning_joint_INFO_',subject,'_',...
        mtp_jointType,'.mat']);
...
if analyseResults
    ...
    for i = 1:N
        ...
        Tau_passk_opt.mtp.l = full(f_passiveTATorques(stiffnessMtp, ...
            dampingMtp, q_opt_unsc_all.rad(i+1,jointi.mtp.l), ...
            qdot_opt_unsc_all.rad(i+1,jointi.mtp.l)));
        Tau_passk_opt.mtp.r = full(f_passiveTATorques(stiffnessMtp, ...
            dampingMtp, q_opt_unsc_all.rad(i+1,jointi.mtp.r), ...
            qdot_opt_unsc_all.rad(i+1,jointi.mtp.r)));       
    ...
    if assertMtpTmaxk > 1*10^(-tol_ipopt)
        disp(['Issue when reconstructing mtp residual forces: mesh points; '...
            'max error: ', num2str(assertMtpTmaxk)])
    end 
    ...
    for i = 1:d*N
        ...
        Tau_passj_opt.mtp.l = full(f_passiveTATorques(stiffnessMtp, ...
            dampingMtp, q_col_opt_unsc.rad(i,jointi.mtp.l), ...
            qdot_col_opt_unsc.rad(i,jointi.mtp.l)));
        Tau_passj_opt.mtp.r = full(f_passiveTATorques(stiffnessMtp, ...
            dampingMtp, q_col_opt_unsc.rad(i,jointi.mtp.r), ...
            qdot_col_opt_unsc.rad(i,jointi.mtp.r)));       
    ...
    if assertMtpTmaxj > 1*10^(-tol_ipopt)
        disp(['Issue when reconstructing mtp residual forces: collocation '...
            'points; max error: ', num2str(assertMtpTmaxj)])
    end 
...
%     plot(Tau_pass_opt_GC(:,jointi.mtp.r-nq.abs) + ...
%         a_mtp_GC(:,2)*scaling.MtpTau, 'm:', 'linewidth', 3)
...
%     set(l,'Fontsize',16)    
    ...
    if saveResults
        ...
        Results_all.(['Case_',num2str(ww)]).StrideLength_opt = ...
            StrideLength_opt;
        Results_all.(['Case_',num2str(ww)]).StepWidth_opt = ...
            stride_width_mean;
        ...
        save([pathresults,'/',namescript,...
            '/Results_all_v2_mtp.mat'], 'Results_all');
    end    
end
end
